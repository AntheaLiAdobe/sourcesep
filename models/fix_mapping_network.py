...
# network that predicts color and transformation with fixed mapping.
class FixMapAffineMLPNet(pl.LightningModule):

    def __init__(self, args, learning_rate=1e-3, batch_size=32, num_points = 2048, bottleneck_size=64, nb_primitives = 1, output_dim=2, points=None):
        super().__init__()
        super(FixMapAffineMLPNet, self).__init__()
        self.args = args
        ...
        self.scale = points[:,:2].max(0)[0] - points[:,:2].min(0)[0] 
        self.rand_grid = Variable(torch.cuda.FloatTensor(1, 2 ,args.num_points//args.nb_primitives))
        self.rand_grid.data.uniform_(0,1) 
        self.rand_grid = self.rand_grid * self.scale.view(1, 2, 1)
        self.mapping = self.get_mapping( points.view(1, -1, 5), self.rand_grid)
        # remove scaling 
    ...
    def get_mapping(self, points1, grid):
        gt_shape1 = points1[:,:,:2]
        grid_shape = grid.transpose(1,2)[:,:,:2]

        dist1_1, dist1_2, idx1_1, idx1_2 = self.chamfer2D(gt_shape1, grid_shape)

        mapping = (idx1_1.repeat(self.batch_size, 1), idx1_2.repeat(self.batch_size, 1))
    ...
    def loss(self, points1, output1, points2, output2, s=1.0, delta=1.0):
        ...
        idx1_1, idx1_2 = self.mapping
        idx2_1, idx2_2 = self.mapping
        ...
        if self.args.chamfer5d_loss:
            ...
            idx1_2 = torch.clamp(idx1_2, min=0, max=gt_color1.shape[1])
            idx2_2 = torch.clamp(idx2_2, min=0, max=gt_color2.shape[1])

            loss1 = torch.pow(scaled_points1.gather(1, idx1_2.unsqueeze(-1).repeat(1,1,5).long()) - scaled_output1, 2)
            loss2 = torch.pow(scaled_points2.gather(1, idx2_2.unsqueeze(-1).repeat(1,1,5).long()) - scaled_output2, 2)

            loss = ( loss1 + loss2 ).mean() * self.args.shape_loss_weight
            batch_loss['chamfer5d_loss'] = loss1.mean().item() + loss2.mean().item() 
            ...
            shape_loss1 = torch.pow(gt_shape1.gather(1, idx1_2.unsqueeze(-1).repeat(1,1,2).long()) - output_shape_1, 2)
            shape_loss2 = torch.pow(gt_shape2.gather(1, idx2_2.unsqueeze(-1).repeat(1,1,2).long()) - output_shape_2, 2)

            loss = ( shape_loss1 + shape_loss2 ).mean() * self.args.shape_loss_weight
            batch_loss['shape_loss'] = shape_loss1.mean().item() + shape_loss2.mean().item() 
            batch_loss['pairing'] = { 'pairing1': (idx1_1, idx1_2), 'pairing2': (idx2_1, idx2_2)}
            ...
            if not self.args.no_color_loss:
                ...
                color_loss1 = torch.pow(gt_color1.gather(1, idx1_2.unsqueeze(-1).repeat(1,1,3).long()) - output_color_1, 2)
                color_loss2 = torch.pow(gt_color2.gather(1, idx2_2.unsqueeze(-1).repeat(1,1,3).long()) - output_color_2, 2)

                loss += ( color_loss1 + color_loss2 ).mean() * self.args.color_loss_weight

                batch_loss['color_loss'] = color_loss1.mean().item() + color_loss2.mean().item()
